...
def register_tiling(
    ...
    i_thread_outer, i_thread_inner = sch.split(i, factors=[None, thread_tile_x])
    j_thread_outer, j_thread_inner = sch.split(j, factors=[None, thread_tile_y])
    k_thread_outer, k_thread_inner = sch.split(k, factors=[None, thread_tile_k])
    ...
    sch.compute_at(A_thread, k_thread_outer)
    B_thread = sch.cache_read(block_gemm, read_buffer_index=1, storage_scope="local")
    sch.compute_at(B_thread, k_thread_outer)
...
def cooperative_fetching(
    ...
    def _cooperative_fetching_impl(block: BlockRV):
        ...
        _, ii, jj  = sch.split(fused_loop, [None, thread_extent_x, thread_extent_y])
...
def write_cache(sch: tir.Schedule) -> None:
    ...
    sch.reverse_compute_at(C_thread_local,  write_cache_loc)
